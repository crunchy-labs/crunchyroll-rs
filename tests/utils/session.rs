...
pub static SESSION: Store<Crunchyroll> = Store::new(|| Box::pin(async {
    ...
    let crunchy = match token_type {
        "refresh_token" => Crunchyroll::new().login_with_refresh_token(token.into()).await.unwrap(),
        "etp_rt" => Crunchyroll::new().login_with_etp_rt(token.into()).await.unwrap(),
...
pub async fn set_session(crunchy: Crunchyroll) -> anyhow::Result<()> {
    match crunchy.session_token().await {
        SessionToken::RefreshToken(refresh_token) => {
            Ok(set_store("session".into(), format!("refresh_token:{}", refresh_token))?)
        }
