...
impl Executor {
    pub(crate) async fn request<T: Request + DeserializeOwned>(self: &Arc<Self>, mut builder: RequestBuilder) -> Result<T, CrunchyrollError> {
        ...
        if config.session_expire <= Utc::now() {
            let login_response = match config.session_token.clone() {
                SessionToken::RefreshToken(refresh_token) => Executor::auth_with_refresh_token(self.client.clone(), refresh_token).await?,
                SessionToken::EtpRt(etp_rt) => Executor::auth_with_etp_rt(self.client.clone(), etp_rt).await?
            };
            ...
            new_config.session_token = match new_config.session_token {
                SessionToken::RefreshToken(_) => SessionToken::RefreshToken(login_response.refresh_token),
                SessionToken::EtpRt(_) => SessionToken::EtpRt(login_response.refresh_token)
            };
            new_config.session_expire = Utc::now().add(Duration::seconds(login_response.expires_in as i64));

            *config = new_config;
        }

        builder = builder.
            bearer_auth(config.access_token.clone());
    ...
    async fn auth_with_refresh_token(client: reqwest::Client, refresh_token: String) -> Result<AuthResponse> {
...
impl CrunchyrollBuilder {
    ...
    pub async fn login_with_refresh_token(self, refresh_token: String) -> Result<Crunchyroll> {
        let login_response = Executor::auth_with_refresh_token(self.client.clone(), refresh_token).await?;
        let session_token = SessionToken::RefreshToken(login_response.refresh_token.clone());
    ...
    async fn post_login(self, login_response: AuthResponse, session_token: SessionToken) -> Result<Crunchyroll> {
        ...
        let crunchy = Crunchyroll{
            executor: Arc::new(Executor {
                ...
                config: Mutex::new(ExecutorConfig {
                    ...
                    session_token,
                    session_expire: Utc::now().add(Duration::seconds(login_response.expires_in as i64))
                }),
                details: ExecutorDetails {
                    locale: self.locale,
